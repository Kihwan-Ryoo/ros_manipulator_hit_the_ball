...
#include "ball_track_alvar/ConnectedComponents.h"
#include "ball_track_alvar/Draw.h"
#include <cassert>
...
LabelingCvSeq::LabelingCvSeq() : _n_blobs(0), _min_edge(20), _min_area(20000)
{
	SetOptions();
...
void LabelingCvSeq::LabelSquares(IplImage* image, bool visualize)
{
    ...
    IplImage* imgHSV;
    CvMat* mask;
    imgHSV = cvCreateImage(cvSize(image->width, image->height), IPL_DEPTH_8U, 3);
    int hmin = 0, smin = 110, vmin = 153;
    int hmax = 19, smax = 240, vmax = 255;
    cvCvtColor(image, imgHSV, CV_BGR2HSV);
    cvInRangeS(imgHSV, CvScalar(hmin, smin, vmin), CvScalar(hmax, smax, vmax), gray);
    //cvCopy(image, image, mask);

    // Convert grayscale and threshold --- deleted part
    ...
    cvFindContours(bw, storage, &contours, sizeof(CvContour), CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE, cvPoint(0,0));
    ...
    {   
        ...
        if( result->total > 6 && CheckBorder(result, image->width, image->height) && 
        ...
        {  
            cout << fabs(cvContourArea(result, CV_WHOLE_SEQ)) << "\n";
            cvSeqPush(squares, result);
            cvSeqPush(square_contours, contours);									
        }
        contours = contours->h_next; // h_next = Next sequence
    }
