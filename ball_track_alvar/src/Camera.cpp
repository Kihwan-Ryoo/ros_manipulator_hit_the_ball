...
#include "ball_track_alvar/Alvar.h"
#include "ball_track_alvar/Camera.h"
#include "ball_track_alvar/FileFormatUtils.h"
#include <memory>
...
void Camera::CalcExteriorOrientation(vector<CvPoint3D64f>& pw, vector<PointDouble>& pi, CvMat *rodriques, CvMat *tra) // 그래서 여기까지 해당한다! (only in image coordinates)
{
	//assert(pw.size() == pi.size());

	int size = (int)pi.size(); // = 4
...
// Marker* mn = _track_markers_at(ii);															in MarkerDetector.cpp
// mn->UpdatePose(blob_corners[track_i], cam, track_orientation, update_pose);					in MarkerDetector.cpp
// if (update_pose) cam->CalcExteriorOrientation(marker_corners, marker_corners_img, &pose);	in Marker.cpp
void Camera::CalcExteriorOrientation(vector<PointDouble>& pw, vector<PointDouble>& pi, CvMat *rodriques, CvMat *tra) // 그래서 여기에 해당한다..!!
{
	//assert(pw.size() == pi.size());
	int size = (int)pi.size(); // = 4
...
void Camera::CalcExteriorOrientation(vector<PointDouble>& pw, vector<PointDouble>& pi, Pose *pose) // 여기에 해당한다..?!
{
	// cam->CalcExteriorOrientation(marker_corners, marker_corners_img, &pose);		from Marker.cpp  Marker::UpdatePose		핵심 부분!!!!!
	// pw = Marker corners in marker coordinates (SetMarkerSize에서 정해진다.) --- marker coordinates
	// pi from blob_corners[i] --- image coordinates
...
Homography::Homography() {
	...
	/* cvMat 형태의 행렬 구조체를 미리 정의된 배열 값으로 초기화 시켜주는 함수
...
{
	assert(pw.size() == pi.size()); // error.. why..
	int size = (int)pi.size(); // 4 (number of marker corners ?)
