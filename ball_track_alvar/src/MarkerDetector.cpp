...
namespace alvar {
			   ...
			   bool update_pose)
	{
		...
		IplImage* gray = labeling->gray; // Pointer to grayscale image that is thresholded for labeling
		...
		if (track) {
			for (size_t ii = 0; ii < _track_markers_size(); ii++) {
				...
				cout << mn->GetError(Marker::DECODE_ERROR | Marker::MARGIN_ERROR) << "\n";
				int track_i = -1;
		...
		cout << "blob_corners.size() = " << blob_corners.size() << "\n";
